...
import { Calendar, BookOpen, Clock, Download, Eye, Bookmark, BookmarkCheck, Share2 } from "lucide-react";
...
export function QuestionCard({ question, isSelected, onSelect, isBookmarked, onToggleBookmark }: QuestionCardProps) {
                ...
                <CardFooter className="p-4 pt-0 text-xs text-muted-foreground flex justify-between items-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                    ...
                    <div className="flex items-center gap-2">
                        <Button
                            variant="ghost"
                            size="icon"
                            className={cn("h-7 w-7 rounded-full", isBookmarked && "text-yellow-500")}
                            onClick={(e) => {
                                e.preventDefault();
                                e.stopPropagation();
                                onToggleBookmark?.();
